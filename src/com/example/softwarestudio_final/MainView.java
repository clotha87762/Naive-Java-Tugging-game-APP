...
public class MainView extends SurfaceView implements Callback {
	...
	/*
	 * Bitmap startButton; Bitmap helpButton; Bitmap optionButton; Bitmap
	 * mainBack; Bitmap rope;
	 */
	Movable startButton;
	Movable helpButton;
	...
	public MainView(MainActivity main) {
		...
		Timeline.createSequence()
				.pushPause(1f)
				.push(Tween.to(title, MovableAccessor.POSITION_Y, 0.4f).target(
						2000f))
				.pushPause(0.2f)
				.push(Tween.to(title, MovableAccessor.POSITION_Y, 0.4f).target(
						-500f))
				...
				.push(Tween.to(title, MovableAccessor.POSITION_Y, 1.0f).target(
						0f))
				.beginParallel()
				.push(Tween.to(startButton, MovableAccessor.POSITION_Y, 0.8f)
						.target(500f))
				.push(Tween.to(startButton, MovableAccessor.SCALE, 0.8f)
						.target(1))
				.end()
				.beginParallel()
				.push(Tween.to(helpButton, MovableAccessor.POSITION_Y, 0.8f)
						.target(930f))
				.push(Tween.to(helpButton, MovableAccessor.SCALE, 0.8f).target(
						1))
				.end()
				.beginParallel()
				.push(Tween.to(optionButton, MovableAccessor.POSITION_Y, 0.8f)
						.target(1358f))
				.push(Tween.to(optionButton, MovableAccessor.SCALE, 0.8f)
						.target(1)).end().start(tm);
	...
	public void draw(Canvas canvas) {
		 ...
		 * canvas.drawBitmap(startButton,MainMenuOffset
		 * [0][0],MainMenuOffset[0][1],p);
		 * canvas.drawBitmap(helpButton,MainMenuOffset
		 * [1][0],MainMenuOffset[1][1],p);
		 * 
		 * canvas.drawBitmap(optionButton,MainMenuOffset[2][0],MainMenuOffset[2][
		 * 1],p);
		 */
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (!isAnimationRunning) {
			...
			if (xx >= MainMenuOffset[0][0]
					&& xx <= MainMenuOffset[0][0]
							+ startButton.bitmap.getWidth()
					&& yy >= MainMenuOffset[0][1]
					&& yy <= MainMenuOffset[0][1]
							+ startButton.bitmap.getHeight()) {
				if (Constant.soundOn)
				...
				if (Constant.onePlayer) {
					...
					builder.setMessage("Please Select a Difficulty for 1P Mode");
			...
			} else if (xx >= MainMenuOffset[1][0]
					&& xx <= MainMenuOffset[1][0]
							+ helpButton.bitmap.getWidth()
					&& yy >= MainMenuOffset[1][1]
					&& yy <= MainMenuOffset[1][1]
							+ helpButton.bitmap.getHeight()) {
				if (Constant.soundOn)
					mainActivity.soundUtil.playEffectsSound(4, 0);
				mainActivity.myHandler.sendEmptyMessage(3);
			} else if (xx >= MainMenuOffset[2][0]
					&& xx <= MainMenuOffset[2][0]
							+ optionButton.bitmap.getWidth()
					&& yy >= MainMenuOffset[2][1]
					&& yy <= MainMenuOffset[2][1]
							+ optionButton.bitmap.getHeight()) {
				if (Constant.soundOn)
					mainActivity.soundUtil.playEffectsSound(4, 0);
